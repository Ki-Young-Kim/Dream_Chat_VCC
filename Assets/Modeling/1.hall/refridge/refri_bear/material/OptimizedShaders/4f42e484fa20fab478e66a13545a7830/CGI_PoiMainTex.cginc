...
void initTextureData(inout float4 albedo, inout float4 mainTexture, inout float3 backFaceEmission, inout float3 dissolveEmission, in half3 detailMask)
{
    ...
    mainTexture.a *= alphaMask;
        ...
        albedo = float4(mainTexture.rgb * max(float4(0.196991,0.05555367,0.01921264,1).rgb, float3(0.000000001, 0.000000001, 0.000000001)) * lerp(1, vertexColor, (0.0 /*_MainVertexColoring*/)), mainTexture.a * max(float4(0.196991,0.05555367,0.01921264,1).a, 0.0000001));
