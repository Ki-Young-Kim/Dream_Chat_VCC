...
bool clearcoatDoesReflectionProbeExist()
{
	...
	return probeExists && !(0.0 /*_ClearcoatForceFallback*/);
}
...
{
	float roughness = 1 - ((0.0 /*_ClearcoatGlossiness*/) * clearcoatMap.a);
	roughness = clamp(roughness, 0.0045, 1.0);
	roughness = roughness * roughness;
	float reflectivity = (1.0 /*_Clearcoat*/) * clearcoatMap.r;
...
float3 getClearcoatDirectSpecular(float roughness, float ndh, float vdn, float ndl, float ldh, float3 f0, float3 halfVector, float3 tangent, float3 bitangent, float anisotropy)
{
		...
		return lerp(directSpecularNonAniso, directSpecularAniso, saturate(abs((0.0 /*_ClearcoatAnisotropy*/) * 100))) * 3; // * 100 to prevent blending, blend because otherwise tangents are fucked on lightmapped object
	#else
		return 0;
	#endif
}
...
{
	#if defined(PROP_CLEARCOATMAP) || !defined(OPTIMIZER_ENABLED)
		float4 clearCoatMap = POI2D_SAMPLER_PAN(_ClearcoatMap, _MainTex, poiMesh.uv[(0.0 /*_ClearcoatMapUV*/)], float4(0,0,0,0));
	#else
	...
	if ((0.0 /*_ClearcoatInvertSmoothness*/))
	{
		clearcoatRoughness = 1 - clearcoatRoughness;
	}
	float3 creflViewDir = getClearcoatAnisotropicReflectionVector(poiCam.viewDir, poiMesh.binormal, poiMesh.tangent.xyz, poiMesh.normals[0], clearcoatRoughness, (0.0 /*_ClearcoatAnisotropy*/));
	...
	float3 vDirectSpecular = 0;
		...
		for (int index = 0; index < 4; index++)
		{
			...
			float3 v0directSpecular = getClearcoatDirectSpecular(clearcoatRoughness, vcndh, max(cvdn, 0.000001), vcndl, saturate(poiLight.vDotLH[index]), clearcoatf0, poiLight.halfDir, poiMesh.tangent, poiMesh.binormal, (0.0 /*_ClearcoatAnisotropy*/)) * poiLight.vAttenuation * vcndl * poiLight.vColor[index];
			vDirectSpecular += v0directSpecular;
		}
	#endif
	float3 clearcoatDirectSpecular = getClearcoatDirectSpecular(clearcoatRoughness, cndh, max(cvdn, 0.000001), attenuation, saturate(poiLight.lDotH), clearcoatf0, poiLight.halfDir, poiMesh.tangent, poiMesh.binormal, (0.0 /*_ClearcoatAnisotropy*/)) * poiLight.attenuation * attenuation * poiLight.color;
	...
	float3 clearcoat = ((clearcoatDirectSpecular + vDirectSpecular) * clearCoatMap.g * (1.0 /*_ClearcoatEnableSpecular*/) + clearcoatIndirectSpecular * clearCoatMap.b * (1.0 /*_ClearcoatEnableReflections*/)) * clearcoatReflectivity * clearcoatFresnel;
